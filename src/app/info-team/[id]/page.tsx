          ...
          <div className="bg-white border border-gray-300 rounded-lg shadow-md p-10 w-full max-w-6xl">
            ...
            <div className="flex flex-col text-start w-[90%]">
              <div className='flex justify-between'>
                <h2 className="text-xl font-bold text-left text-black ">Nombre equipo/proyecto:{team.team_name}</h2>
                <div>
                  <h3 className='font-bold'>{new Date(team.finish_date).toLocaleDateString()}</h3>
                  <h3 className='font-bold'>{new Date(team.created_date).toLocaleDateString()}</h3>
                </div>
              </div>
              ...
              <div>
                ...
                <div className='flex justify-evenly '>
                  <div className='flex flex-col justify-center items-center w-2/4'>
                    ...
                    <Image src={user.profilePicture} alt='icono' height={80} width={80} className='w-[80px] h-[80px] rounded-[50%]'></Image>
                    <h3 className='p-1'>{team.team_leader}</h3>
                  </div>
                  <div className='flex flex-col justify-evenly flex-wrap items-center w-2/4'>
                  <h3 className='font-bold p-2'>Colaboradores:{team.team_users}</h3>
                  <Image src={user.profilePicture} alt='icono' height={80} width={80} className='w-[80px] h-[80px] rounded-[50%]'></Image>
                  </div>
                </div>
              </div>
